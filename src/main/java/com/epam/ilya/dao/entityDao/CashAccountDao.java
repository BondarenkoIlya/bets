...
public class CashAccountDao extends Dao implements EntityDao<CashAccount> {
    ...
    public CashAccount findByPerson(Person person) throws DaoException {
        ...
        try {
            PreparedStatement statement = getConnection().prepareStatement("select cash_accounts.id , cash_accounts.balance from cash_accounts join "+ tableName+ " on cash_accounts.id="+ tableName+".purse_id where "+tableName+".id=?");
